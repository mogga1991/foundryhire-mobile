/**
 * SQL migrations to create optimized indexes for query-heavy routes
 *
 * These indexes are designed to improve performance for:
 * - Interview listing with complex filters
 * - Candidate listing with search and sorting
 * - Campaign listing and tracking
 * - Dashboard aggregation queries
 *
 * Run via database migration tool or admin endpoint
 */

export const PERFORMANCE_INDEX_MIGRATIONS = `
-- =============================================================================
...
/**
 * SQL to drop all performance indexes (for rollback)
 */
export const DROP_PERFORMANCE_INDEXES = `
-- Interviews
DROP INDEX IF EXISTS idx_interviews_company_status_scheduled;
DROP INDEX IF EXISTS idx_interviews_candidate_scheduled;
DROP INDEX IF EXISTS idx_interviews_job_scheduled;
DROP INDEX IF EXISTS idx_interviews_upcoming;
DROP INDEX IF EXISTS idx_interviews_recording_processing;
DROP INDEX IF EXISTS idx_interviews_scheduled_by;

-- Candidates
DROP INDEX IF EXISTS idx_candidates_company_job_status_score;
DROP INDEX IF EXISTS idx_candidates_company_created;
DROP INDEX IF EXISTS idx_candidates_enrichment_status;
DROP INDEX IF EXISTS idx_candidates_verified;
DROP INDEX IF EXISTS idx_candidates_status_company;

-- Campaigns
DROP INDEX IF EXISTS idx_campaigns_job_company_created;
DROP INDEX IF EXISTS idx_campaigns_active;
DROP INDEX IF EXISTS idx_campaigns_status_created;
DROP INDEX IF EXISTS idx_campaigns_created_by;

-- Campaign Sends
DROP INDEX IF EXISTS idx_campaign_sends_campaign_status;
DROP INDEX IF EXISTS idx_campaign_sends_candidate;
DROP INDEX IF EXISTS idx_campaign_sends_pending;
DROP INDEX IF EXISTS idx_campaign_sends_opened;
DROP INDEX IF EXISTS idx_campaign_sends_clicked;

-- Activities
DROP INDEX IF EXISTS idx_activities_candidate_created;
DROP INDEX IF EXISTS idx_activities_company_created;
DROP INDEX IF EXISTS idx_activities_type_created;
DROP INDEX IF EXISTS idx_candidate_activities_performed_by;
